test_that("Endpoints are properly identified", {
  r <- PlumberRouter$new("files/endpoints.R")
...
test_that("The file is sourced in the envir", {
  r <- PlumberRouter$new("files/in-env.R")
  expect_equal(length(r$endpoints), 1)
...
test_that("Verbs translate correctly", {
  r <- PlumberRouter$new("files/verbs.R")
  expect_equal(length(r$endpoints), 1)
...
test_that("Invalid file fails gracefully", {
  expect_error(PlumberRouter$new("asdfsadf"), regexp="File does not exist.*asdfsadf")
})

test_that("Empty endpoints error", {
  expect_error(PlumberRouter$new("files/endpoints-empty.R"), regexp="No path specified")
})
