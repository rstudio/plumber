...
test_that("content type", {
  ...
  val2<-serializer_content_type(type="text/plain")(
                                              readBin(tmp, "raw", n=file.info(tmp)$size),list(),PlumberResponse$new(), stop)


  expect_equal(val2$status, 200L)
  expect_equal(val2$header$`Content-Type`,"text/plain")
  expect_equal(val2$body,readBin(tmp, "raw", n=file.info(tmp)$size))
})

test_that("content type disposition error", {
