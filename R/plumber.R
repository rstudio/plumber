...
Plumber <- R6Class(
  ...
  public = list(
    ...
    #' @description Add an object definition to plumber router
    #'
    #' See also: [pr_object()]
    #' @param name a character string. Name of object
    #' @param props list of properties
    object = function(name, props){
      private$addObjectInternal(name, props)
    },
    #' @description
    ...
    getApiSpec = function() { #FIXME: test
      ...
      # Extend again with any defined objects
      components <- private$componentsSpecificationInternal(self)
      if (!is.null(components)) {
        def <- utils::modifyList(def, list(components = components))
      }
    ...
    #' @field objects Plumber router objects read-only
    objects = function(){
      private$objs
    },
  ...
  private = list(
    ...
    objs = list(), # List of object definitions indexed by their name
    mnts = list(),
    ...
    addObjectInternal = function(name, props) {
      # Create a new object definition and add it to the router
      toAdd <- list()
      toAdd[[name]] <- list(name = name, props = props)
      private$objs <- c(private$objs, toAdd)
      self
    },
    ...
    componentsSpecificationInternal = function(router) {
      objectsList <- list()

      for (object in router$objects) {
        objectSpec <- objectSpecification(object)
        objectsList <- utils::modifyList(objectsList, objectSpec)
      }

      if (length(objectsList) == 0) return(NULL)

      return(list(schemas = objectsList))
    },
