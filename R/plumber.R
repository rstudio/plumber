...
plumber <- R6Class(
  ...
  public = list(
    ...
    #' @param swagger Deprecated. Use `plumber` method `setUI` and `apiSpecHandler`.
    ...
    run = function(
      ...
      if (!missing(swagger)) {
        warning("`swagger` parameter has been deprecated in v1.0.0 and will be removed in a coming release. Please use `$setUI()` and `$apiSpecHandler()`.")
        if (is.function(swagger)) {
          self$setUI(TRUE)
          self$apiSpecHandler(swagger)
        } else {
          self$setUI(swagger)
        }
      }
    ...
    apiSpecHandler = function(api_fun = NULL) {
      if (is.null(api_fun)) {
        api_fun <- identity
      }
      if (!is.function(api_fun)) {
        stop("`api_fun` must be a function")
      }
