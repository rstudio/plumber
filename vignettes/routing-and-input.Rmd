...
`postBodyParsed` | `list(a=1,b=2)` | The parsed form of `postBodyRaw`
...
If multiple parameters are matched to the endpoint formals, an error will be thrown. Due to the nature of how multiple values can be matched to the same argument, it is recommended that `POST` enpoints have a function definition that only accepts the formals `req`, `res`, and `...`. If the endpoing arguments are to be processed like a list, they are available at `req$postBodyParsed`, with all arguments at `req$args`. Do try to avoid using `req` and `res` as path or query string names as Plumber's `req` and `res` will overwrite any values present in `req$args`.
